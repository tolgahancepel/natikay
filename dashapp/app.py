...
mongo_client = pymongo.MongoClient("<your updated connection string>")  # you need to edit here
...
card_temperature = dbc.Card(
    [
        ...
        html.P(id="current-temperature", className="text-temperature"),
...
card_humidity = dbc.Card(
    [
        ...
        html.P(id="current-humidity", className="text-humidity"),
...
card_air = dbc.Card(
    [
        dbc.Card(
            [
                html.Img(src="/assets/water-bottle.svg", className="img-infocard")
            ], className="card-air"
        ),

        html.P(id="current-wlevel", className="text-air"),
        html.P("Water Level (%)", className="text-infocard-unit"),
...
@app.callback(
    [
        ...
        Output('current-wlevel', 'children'),
    ],
...
def update_soil_moisture(timer, selected_value):
    ...
    colors = ['#D84315', '#FF8F00', '#9E9D24', '#2E7D32']
    ...
    df = pd.DataFrame(list(sensor_history.find()))
    df['datetime'] = pd.to_datetime(df['timestamp'], unit='s', infer_datetime_format=True)

    current_temperature = df['temperature'].iloc[-1]
    current_humidity = df['humidity'].iloc[-1]
    current_wlevel = df['wlevel'].iloc[-1]
    ...
    fig_infocard.add_trace(
        go.Scatter(x=df["datetime"], y=df[selected_value], mode='lines+markers', marker_size=8,
            name=labels[2],
            line=dict(color=infocard_marker_color, width=line_size[2]),
            connectgaps=True,
            marker=dict(size=6, symbol="triangle-right")
        )
    )
    ...
    somo_list = ["somo1", "somo2", "somo3", "somo4"]

    for i in range(4):
        fig_soil_moisture.add_trace(go.Scatter(x=df["datetime"], y=df[somo_list[i]], mode='lines',
            name=labels[i],
        ...
        fig_soil_moisture.add_trace(go.Scatter(
            ...
            y=df[somo_list[i]],
            mode='lines+markers',
            marker=dict(color=colors[i], size=mode_size[i], symbol="triangle-right")
        ))
    ...
    return fig_infocard, infocard_style, fig_soil_moisture, current_temperature, current_humidity, current_wlevel
